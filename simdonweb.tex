...
operations.
%
The current set of benchmarks cover these application domains:
...
\begin{itemize}
\item Vertex Transformation: Transformation of 4 element vector by 4x4 matrix
\item Matrix Transpose: Transposition of 4x4 matrix
\item Matrix Inversion: Compute the inverse of a 4x4 matrix
\item Matrix Multiplication: Multiply two 4x4 matrices
\end{itemize}

\item Cryptography
This might not be a typical domain where use of SIMD operations can improve
performance, but we found a hot function in the implementation of the
Rijndael cipher that would benefit, so we extracted it into a benchmark kernel.

\begin{itemize}
\item Shift Rows (shift/rotate of row values in 4x4 matrix)
\end{itemize}

\item Vector Math Computations
...
individual SIMD instructions.  Intel provides a Small Vector Math Library for
that purpose for use in C/C++ SIMD code.  The following benchmark is an
equivalent JS implementation of the sine() function. It is envisioned that such
base math operations (sin, cos, exp, pow, etc) will be useful for things like
signal processing and physics engines.  Showing that basic math operations can
be implemented using the JS SIMD API, with performance improvements equivalent
to that of highly optimized C/C++ functions, shows that JS truly has the
potential to become the implementation language for performance critical code.
